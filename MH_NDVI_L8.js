...
////// Function to add an SAVI band, the dependent variable.
...
var addConstant = function(image) {
  return image.addBands(ee.Image(1)).clip(zonas);
};
...
var harmonicLandsat = L8
  .filterBounds(zonas)
  .filterDate('2014', '2023')
  .map(maskL8sr)
  .map(addNDVI)
  .map(addConstant)
  .map(addTime)
...
print(ui.Chart.image.series(
  fittedHarmonic.select(['fitted', 'NDVI']), ZN, ee.Reducer.median(), 30)
    .setOptions({
      title: 'Harmonic model: original and fitted values ZN',
      lineWidth: 1,
      pointSize: 3
    })
);
...
print(ui.Chart.image.series(
  fittedHarmonic.select(['fitted', 'NDVI']), ZS, ee.Reducer.median(), 30)
    .setOptions({
      title: 'Harmonic model: original and fitted values ZS',
      lineWidth: 1,
      pointSize: 3
    })
);
...
Map.addLayer(harmonicLandsat,
  ...
  'NDVI Mosaic');
Map.addLayer(zonas, {color: 'yellow'}, 'zonas');
